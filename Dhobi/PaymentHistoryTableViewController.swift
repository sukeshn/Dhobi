...
//  Dhobi
//
//  Created by Sukesh Nemane on 17/08/17.
...
class PaymentHistoryTableViewController: UITableViewController {
    ...
    var fetchedResultsController: NSFetchedResultsController<PaymentHistory>?
    

    private func updateUI()
    {
        if let context = container?.viewContext {
            let request: NSFetchRequest<PaymentHistory> = PaymentHistory.fetchRequest()
            request.sortDescriptors = [NSSortDescriptor(key: "datePaid", ascending: true, selector: nil)]
            
            fetchedResultsController = NSFetchedResultsController<PaymentHistory>(fetchRequest: request, managedObjectContext: context, sectionNameKeyPath: nil, cacheName: nil)
